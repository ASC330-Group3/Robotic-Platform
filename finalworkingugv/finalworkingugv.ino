...
// PWM1=kp*(1+(time3/100))*offset1-prevoffset1*kp;
  if(PWM1>0)
  {
    directionM1=0;
  }
...
//  Serial.println(totaloffset1);
  diffPosition1 = Position1;
  time3 = time4;
  prevoffset1=offset1;
  if(xvel==0&&yvel==0&&rot==0)
  {
    PWM1=0;
    totaloffset1=0;
  }
...
static unsigned long timePoint2 = 0;
...
void loop ()
{
  ...
  xvel=0.5;
  yvel=0.5;
  if(millis() - timePoint2 > 15){
   if((xvel>0)&&(urmData[3]<10))
   {
    xvel=0;
    yvel=0;
   }
   if((xvel<0)&&(urmData[1]<10))
   {
    xvel=0;
    yvel=0;
   }
   if((yvel>0)&&(urmData[2]<15))
   {
    xvel=0;
    yvel=0;
   }
   if((yvel<0)&&(urmData[0]<15))
   {
    xvel=0;
    yvel=0;
   }
